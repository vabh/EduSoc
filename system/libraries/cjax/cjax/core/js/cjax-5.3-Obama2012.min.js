...
var script;var name='cjax-5.3-Obama2012.js';var src;script=CJAX.$('cjax_lib');if(script){src=script.src;var f=src.replace(/cjax-.+$/,'');CJAX.uri=script.src;}else{var scripts=document.getElementsByTagName('script');for(var i=0;i<scripts.length;i++){script=scripts[i];src=script.src;if(CJAX.util.get.basename(src).indexOf('cjax-')!=-1){var f=src.replace(/cjax-.+$/,'');CJAX.uri=src;return f;}}}
...
if(plugin_name=CJAX.xml('is_plugin',method)){if(init=CJAX.inits[plugin_name]){fn=CJAX.lib.pharseFunction(init);fn();}
_fn=function(){CJAX._extendPlugin(plugin_name,method,{element:element,event:event,element_id:element.id});};}else{_fn=function(data){_method=data.replace(/\n/g,"");if(CJAX.util.isXML(_method)){if(!CJAX.is_cjax(_method)){_method="<cjax>"+_method+"</cjax>";}
CJAX._process(_method,'set.event',element);}else{eval(_method);}};}
...
var plugins_dir=__base__+'plugins/';var path=plugins_dir+file;var extra={};var file;var params={};if(buffer){file=CJAX.xml('file',buffer);if(_extra=CJAX.decode(CJAX.xml('extra',buffer))){extra=_extra;}
...
CJAX.$('cjax_overlay').style.display='none';}
url=CJAX._pharseValues(url);if(CJAX.ajaxSettings.cache){lower_url=url.toLowerCase();if(CJAX.cache_calls[lower_url]){CJAX.ajaxSettings.cache=false;if(CJAX.callback_success[url]){CJAX.callback_success[url](CJAX.cache_calls[url]);}
...
CJAX.HTTP_REQUEST_INSTANCE.setRequestHeader('X-Requested-With','CJAX FRAMEW0RK 5.3-Obama2012');return full_url;};this._handlerRequestStatus=function(url,status,container)
